...
class TestJoinLeave:
    async def test_join_gives_position(self, db, printers, make_user):
        ...
        first_join = await svc.join(
            db, first.id, MachineKind.PRINTER, now=NOON + timedelta(minutes=1)
        )
        second_join = await svc.join(
            db, second.id, MachineKind.PRINTER, now=NOON + timedelta(minutes=2)
        )
    ...
    async def test_join_twice_is_rejected(self, db, printers, make_user):
        ...
        await svc.join(db, user.id, MachineKind.PRINTER, now=NOON)
    ...
    async def test_leaving_offered_slot_passes_it_on(self, db, printers, make_user):
        ...
        first = await make_user()
        second = await make_user()
        await svc.join(db, first.id, MachineKind.PRINTER, now=NOON + timedelta(minutes=1))
        await svc.join(db, second.id, MachineKind.PRINTER, now=NOON + timedelta(minutes=2))
    ...
    async def test_queue_order_is_fifo(self, db, printers, make_user):
        ...
        for index, person in enumerate(people):
            await svc.join(db, person.id, MachineKind.PRINTER, now=NOON + timedelta(minutes=index))
...
class TestOffers:
    ...
    async def test_queue_is_shared_between_printers(self, db, printers, make_user):
        ...
        await svc.join(db, waiting.id, MachineKind.PRINTER, now=NOON)
    ...
    async def test_two_free_printers_give_two_offers(self, db, printers, make_user):
        ...
        await svc.join(db, first.id, MachineKind.PRINTER, now=NOON + timedelta(minutes=1))
        await svc.join(db, second.id, MachineKind.PRINTER, now=NOON + timedelta(minutes=2))
        await svc.join(db, third.id, MachineKind.PRINTER, now=NOON + timedelta(minutes=3))
    ...
    async def test_join_when_printer_already_free_offers_immediately(
        ...
        result = await svc.join(db, user.id, MachineKind.PRINTER, now=NOON)
...
class TestExpiry:
    ...
    async def test_expired_user_can_join_again(self, db, printers, make_user):
        ...
        again = await svc.join(
            db, waiting.id, MachineKind.PRINTER, now=NOON + timedelta(minutes=40)
        )
...
class TestKinds:
    ...
    async def test_offer_goes_only_to_the_line_of_that_kind(
        ...
        await svc.join(db, wants_engraver.id, MachineKind.ENGRAVER, now=NOON + timedelta(minutes=1))
        await svc.join(db, wants_printer.id, MachineKind.PRINTER, now=NOON + timedelta(minutes=2))

        result = await machines_svc.release(db, owner, printers[0].id, now=NOON)

        assert [offer.user_id for offer in result.offers] == [wants_printer.id]

    async def test_free_engraver_is_offered_at_once(self, db, printers, engravers, make_user):
        """Гравировщик свободен, а человек встаёт в его очередь — приглашение сразу."""
        waiting = await make_user()

        result = await svc.join(db, waiting.id, MachineKind.ENGRAVER, now=NOON)
    ...
    async def test_position_is_counted_inside_its_own_line(
        ...
        await svc.join(db, first_printer.id, MachineKind.PRINTER, now=NOON + timedelta(minutes=1))
        await svc.join(db, second_printer.id, MachineKind.PRINTER, now=NOON + timedelta(minutes=2))
        await svc.join(
            db, engraver_waiter.id, MachineKind.ENGRAVER, now=NOON + timedelta(minutes=3)
        )

        # третий вставший — первый в своей очереди, а не третий в общей
        assert await svc.position_of(db, engraver_waiter.id) == 1
        assert await svc.position_of(db, second_printer.id) == 2

    async def test_one_line_per_person_across_kinds(self, db, printers, engravers, make_user):
        """Правило 2: занять получится только что-то одно, значит и ждать — тоже."""
        await occupy_both(db, printers, make_user)
        await machines_svc.occupy(db, await make_user(), engravers[0].id, 60, now=NOON)
        person = await make_user()
        await svc.join(db, person.id, MachineKind.PRINTER, now=NOON)

        with pytest.raises(AlreadyInQueue):
            await svc.join(db, person.id, MachineKind.ENGRAVER, now=NOON)

    async def test_unknown_kind_is_refused(self, db, printers, make_user):
        person = await make_user()

        with pytest.raises(MachineKindUnknown):
            await svc.join(db, person.id, "лазер", now=NOON)
