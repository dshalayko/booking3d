...
Бронировать можно только то время, когда мастерская открыта, и сетка
расписания показывает ровно эти часы. Проверки идут на настоящем Postgres, как
и весь набор: часы лежат в базе, и то, что строка там ровно одна, держит
ограничение, а не код.

Часы по умолчанию — 08:00–20:00 из миграции 0007; conftest возвращает их к этим
значениям перед каждым тестом.
"""
...
class TestStorage:
    async def test_default_hours_come_from_the_migration(self, db):
        assert await workhours_svc.get(db) == schedule.Hours(time(8, 0), time(20, 0))

    async def test_hours_are_saved(self, db):
        await workhours_svc.save(db, "09:30", "18:00")
        await db.commit()

        assert await workhours_svc.get(db) == schedule.Hours(time(9, 30), time(18, 0))

    async def test_closing_before_opening_is_refused(self, db):
        with pytest.raises(WorkHoursInvalid):
            await workhours_svc.save(db, "20:00", "08:00")

    async def test_garbage_is_refused(self, db):
        with pytest.raises(WorkHoursInvalid):
            await workhours_svc.save(db, "восемь утра", "20:00")

    async def test_round_the_clock_is_allowed(self, db):
        """00:00–00:00 — это круглые сутки, а не окно нулевой длины."""
        hours = await workhours_svc.save(db, "00:00", "00:00")
...
class TestBookingWithinHours:
    ...
    async def test_new_hours_change_what_can_be_booked(self, db, printers, make_user):
        user = await make_user()
        await workhours_svc.save(db, "10:00", "22:00")
        await db.commit()
    ...
    async def test_round_the_clock_lifts_the_limit(self, db, printers, make_user):
        user = await make_user()
        await workhours_svc.save(db, "00:00", "00:00")
        await db.commit()
...
class TestGrid:
    async def test_grid_shows_only_working_hours(self, db, printers):
        park = await machines_svc.list_machines(db, kind=MachineKind.PRINTER)

        grid = await svc.day_schedule(
            db, park, MachineKind.PRINTER, schedule.day_of(at(10)), now=NOON
        )

        assert grid.hours == [f"{hour:02d}:00" for hour in range(8, 20)]
        assert grid.work_hours == schedule.Hours(time(8, 0), time(20, 0))

    async def test_grid_follows_new_hours(self, db, printers):
        await workhours_svc.save(db, "10:00", "14:00")
        await db.commit()
        park = await machines_svc.list_machines(db, kind=MachineKind.PRINTER)
    ...
    async def test_round_the_clock_grid_covers_the_day(self, db, printers):
        await workhours_svc.save(db, "00:00", "00:00")
        await db.commit()
        park = await machines_svc.list_machines(db, kind=MachineKind.PRINTER)
    ...
    async def test_booking_made_before_the_hours_moved_stays_visible(
        ...
        await workhours_svc.save(db, "12:00", "20:00")
        await db.commit()
...
class TestAdminPage:
    ...
    async def test_page_shows_current_hours(self, client, make_user):
    ...
    async def test_form_saves_new_hours(self, client, db, make_user):
        await make_user(is_admin=True)
        await login(client)

        response = await client.post(
            "/admin/hours", data={"opens_at": "09:00", "closes_at": "18:30"}
        )

        assert response.status_code == 303
        assert await workhours_svc.get(db) == schedule.Hours(time(9, 0), time(18, 30))

    async def test_form_refuses_closing_before_opening(self, client, db, make_user):
        await make_user(is_admin=True)
        await login(client)
