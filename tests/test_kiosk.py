...
class TestBoard:
    ...
    async def test_partial_returns_only_the_fragment(self, client, printers):
        response = await client.get("/partials/board")
...
class TestOpenAccess:
    ...
    async def test_queue_join_without_enrolled_device(self, client, db, printers, make_user):
        ...
        response = await client.post("/queue/join/printer", data={"pin": "4242"})
...
class TestReleaseAndQueue:
    ...
    async def test_stranger_cannot_release_while_printing(self, client, db, printers, make_user):
        machine_id = printers[0].id
        owner = await make_user(pin="4242")
    ...
    async def test_join_and_leave_queue(self, client, db, printers, make_user):
        ...
        joined = await client.post("/queue/join/printer", data={"pin": "4242"})
    ...
    async def test_queue_asks_pin_again(self, client, db, printers, make_user):
        ...
        await client.post("/queue/join/printer", data={"pin": "4242"})
    ...
    async def test_flash_message_is_shown(self, client, printers):
        ...
        assert "Машина освобождена" in response.text
...
class TestKindsOnTheWall:
    ...
    async def test_board_has_a_section_per_kind(self, client, printers, engravers):
        response = await client.get("/")

        assert "Принтеры" in response.text and "Гравировщики" in response.text
        assert "Гравёр #1" in response.text

    async def test_each_section_has_its_own_queue_button(self, client, printers, engravers):
        response = await client.get("/")

        assert "/queue/join/printer" in response.text
        assert "/queue/join/engraver" in response.text

    async def test_busy_engraver_says_it_engraves(self, client, db, engravers, make_user):
    ...
    async def test_empty_park_explains_itself(self, client):
    ...
    async def test_join_the_engraver_line_from_the_kiosk(
        ...
        response = await client.post("/queue/join/engraver", data={"pin": "4242"})
    ...
    async def test_unknown_kind_is_refused(self, client, printers):
        ...
        response = await client.get("/queue/join/laser", headers={"accept": "text/html"})
