...
Способов действовать два, и они разделены по устройствам (правило 11).

**Планшет в мастерской, по PIN.** Планшет один раз регистрируется через
`/kiosk/enroll` и получает подписанную device-cookie на 10 лет; без неё форма
ввода PIN не отдаётся вовсе. Это не украшение, а единственное, что защищает PIN:
сервер публичный, а четыре цифры перебираются из интернета за минуты. Сессии у
киоска нет — PIN вводится под каждое действие: планшет общий, и любой перенос
входа между действиями означал бы, что следующий человек либо ждёт, пока чужая
сессия истечёт, либо занимает машину от чужого имени.

**Telegram Mini App, по подписи Telegram.** С телефона личность подтверждает не
PIN, а `initData`, подписанный токеном бота (см. services/telegram.py). Четыре
цифры за пределами мастерской не вводятся нигде, поэтому перебирать нечего.
Проверенное открытие обменивается на подписанную cookie на месяц, чтобы не
проверять подпись на каждый тап.

Доска при этом открыта всем и без всякого входа: смотреть статусы можно откуда
угодно.

Долгоживущие cookie — админская и приложенческая — тоже подписанные токены
(itsdangerous), без строки в БД. Цена решения: отозвать одно устройство нельзя,
только ротацией секрета сразу для всех. При одном-двух планшетах и десятке людей
это дешевле отдельной таблицы.
"""
...
# Сессия Mini App: месяц. Она не даёт ничего, чего не даёт открытие приложения из
# бота, — только избавляет от повторной проверки подписи на каждый тап.
APP_SESSION_TTL = 30 * 24 * 3600
...
def issue_app_session(user_id: int) -> str:
    """Сессия Mini App — тоже подписанный токен, без строки в БД.

    В токене только номер человека: право действовать он подтвердил подписью
    Telegram при открытии приложения (см. services/telegram.py), а всё остальное
    о нём известно из базы.
    """
    return _timed(settings.session_secret, "app-session", "SESSION_SECRET").dumps(user_id)


def app_session_user_id(token: str | None) -> int | None:
    if not token or not settings.session_secret:
        return None
    try:
        user_id = _timed(settings.session_secret, "app-session", "SESSION_SECRET").loads(
            token, max_age=APP_SESSION_TTL
        )
    except (BadSignature, SignatureExpired):
        return None
