...
class Settings(BaseSettings):
    ...
    # Парк принтеров, через запятую. Без дефолта намеренно: имя принтера — это
    # то, что написано на стене конкретного коворкинга, и второй его копии в
    # коде быть не должно. Нет строки в .env — приложение не поднимется, и это
    # лучше, чем парк «P2S #1, P2S #2», взявшийся неизвестно откуда.
