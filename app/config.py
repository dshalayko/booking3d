...
class Settings(BaseSettings):
    ...
    # Имя бота без собаки — из него собирается QR под клавиатурой PIN на
    # планшете (app/qr.py). Из токена его не достать без запроса в Telegram, а
    # киоск должен рисоваться и без сети. Пусто — кнопки с QR нет.
    tg_bot_username: str = ""

    # Снимает привязку ввода PIN к зарегистрированному планшету: выполнять
    # действия с машинами можно с любого устройства. Нужно, чтобы
