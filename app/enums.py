...
class RoomKind(StrEnum):
    ...
    словами о нём говорят экраны. Всё остальное — занятие, освобождение,
    очередь, бронь, журнал — у мастерской и переговорной устроено одинаково, и
...
class MachineKind(StrEnum):
    ...
    занять, освободить, встать в очередь, забронировать и попасть в журнал у
    комнаты и у машины — одно и то же действие. Различает их тип, а слова
    подставляют тексты.
...
# на плитке и в какую очередь встают, и принтер посреди переговорной сделал бы
