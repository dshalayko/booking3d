...
Если сервер облачный, тем же самым — открыть 80 и 443 — надо заняться в панели
провайдера: у VPS обычно есть внешний firewall перед машиной (в Oracle Cloud —
security list у VCN), и `ufw` его не отменяет. Без этого Caddy не получит
сертификат, а в логах будет `Timeout during connect`.
...
- `ports: 127.0.0.1:8000` — приложение доступно с самого сервера, но не из
  интернета напрямую: снаружи всё идёт только через Caddy, и он же остаётся
  единственным местом, где решается, что наружу не отдавать.
...
`PRINTER_NAMES` — имена принтеров для первого запуска, так, как они подписаны
на стене. Дефолта в коде нет намеренно: без этой строки приложение не поднимется
и скажет, чего не хватает. **Если обновляешь развёрнутый сервер**, у которого
`.env` собран до появления этой переменной — допиши её перед перезапуском, иначе
контейнер `app` уйдёт в перезапуск с `не задано в .env: PRINTER_NAMES`. Кавычки
обязательны: `#` без них dotenv считает началом комментария.

Дальше парк живёт в БД и правится из админки, вкладка «Оборудование» — там же
заводятся гравировщики, которых в `PRINTER_NAMES` нет. Правка этой строки уже
созданное не переименует; если админка недоступна, то же самое делает
`docker compose exec app python -m app.cli rename_machine <id> "<новое имя>"`.
...
Админка — `https://printers.example.com/admin/login`, вход по `ADMIN_SECRET`.
Доступна из интернета: администрировать нужно и с телефона, а не только с
ноутбука, на котором поднят туннель.

Чем она при этом защищена и чем нет — в [чек-листе безопасности](#чек-лист-безопасности).
Коротко: секрет в 256 бит, пауза после пяти неудач, cookie на 8 часов, второго
фактора нет. Храни `ADMIN_SECRET` в менеджере паролей, не пересылай его в
переписке и не открывай админку с чужих устройств.

Туннель продолжает работать и остаётся способом зайти, если админку закрыли
обратно или что-то не так с сертификатом:

```bash
ssh -L 8000:localhost:8000 booking@printers.example.com
```

Дальше `http://localhost:8000/admin` в браузере.
...
2. **Поделиться → На экран «Домой»** — и дальше открывать **только с этой
   иконки**. Так пропадают адресная строка и панель вкладок Safari, а страница
   растягивается под самый верх экрана. Открытый в самом Safari сайт останется с
   его панелями, даже в гид-доступе.
3. **Настройки → Экран и яркость → Автоблокировка → Никогда.** Страница вдобавок
   сама держит экран через Wake Lock, но на это одно полагаться нельзя: работает
   он только по https и с iPadOS 16.4. И проверь, что **не включён режим
   энергосбережения** — он возвращает автоблокировку через 30 секунд, что бы ни
   стояло в настройках.
...
**Про полосу с часами сверху.** Это системный статус-бар iPadOS, и убрать его
страница не может — такого API у веба нет ни в Safari, ни у PWA с домашнего
экрана, ни в гид-доступе. Что сделано вместо этого: страница занимает и его
полосу тоже (`viewport-fit=cover` плюс `black-translucent` в `base.html`), так
что часы и батарея рисуются прямо поверх тёмного фона табло, без отдельной
светлой планки, а содержимое под них не заезжает — отступы считаются от
`env(safe-area-inset-*)`. Полноэкранный режим (`requestFullscreen`) не подходит:
он сбрасывается на каждом переходе, а тут переход после каждой кнопки.

Если полоса мешает принципиально, остаётся только не-веб путь: kiosk-браузер из
App Store, умеющий прятать статус-бар, или свой WKWebView-обёртка. Оба открывают
тот же адрес, и киоск-cookie в них ставится тем же шагом 1.

Пока планшета нет под рукой, а проверить цикл на живом сервере надо, — в `.env`
есть `KIOSK_OPEN_ACCESS=true`: он снимает привязку ввода PIN к устройству, и
занимать принтеры можно с телефона или ноутбука. Это режим на время проверки:
PIN — четыре цифры, а адрес публичный, и от перебора остаётся только пауза после
пяти неудач с одного адреса. Выключай сразу после тестов:

```bash
sed -i 's/^KIOSK_OPEN_ACCESS=.*/KIOSK_OPEN_ACCESS=false/' .env
docker compose -f docker-compose.prod.yml up -d --force-recreate app
```

Проверить, в каком режиме сервер сейчас, можно по логам старта — пока флаг
включён, там на каждом запуске висит `KIOSK_OPEN_ACCESS включён`.
...
Это всё. Скрипт делает по порядку: проверяет, что рабочее дерево чистое и docker
отвечает; показывает, какие коммиты сейчас раскатятся; снимает бэкап базы и
падает, если тот вышел пустым; подтягивает код (`--ff-only`, чтобы разошедшаяся
история не превратилась в merge-коммит на проде); пересобирает `app`, а при
изменении `docker-compose.prod.yml` — все сервисы; ждёт `healthz` до 90 секунд.

Что он ловит такого, что руками забывается:

- **бэкап перед миграцией** — не «если вспомнил», а всегда;
- **`Caddyfile`** — если менялся, пересоздаёт `caddy` (почему это нужно — ниже);
- **новые переменные в `.env.example`** — сравнивает с твоим `.env` и называет
  недостающие, пока контейнер не ушёл в цикл рестарта с `не задано в .env: …`;
- **приложение не поднялось** — печатает хвост лога и готовые команды откката
  кода и восстановления базы из только что снятого бэкапа;
- **менялись шаблоны или статика** — напоминает перезагрузить страницу на iPad:
  уже открытая работает на старом JS, пока её не обновить руками.

Простой — около 10 секунд, миграции накатывает контейнер `app` при старте.
Старые бэкапы чистятся по возрасту (14 дней), висячие образы после пересборок —
`docker image prune`.

Пересобрать текущий коммит, не подтягивая ничего: `./deploy.sh --force`.

Если что-то из этого нужно сделать руками (скрипт сломан, отлаживаешь на месте):
...
**Если в обновлении менялся `Caddyfile`** — этого мало, нужен ещё один шаг:

```bash
docker compose -f docker-compose.prod.yml up -d --force-recreate caddy
```

Причина неочевидная: в compose `Caddyfile` смонтирован отдельным файлом, а
`git pull` не правит файл на месте, а заменяет его целиком — новым inode.
Монтирование продолжает показывать старый, поэтому внутри контейнера остаётся
прежний конфиг, и `caddy reload` честно отвечает `config is unchanged`. Снаружи
это выглядит как «правка не применилась без причины». Пересоздание контейнера
заново связывает файл. (Другой способ — монтировать каталог, а не файл: замена
файла внутри каталога контейнеру видна.) `deploy.sh` делает этот шаг сам.
...
- [ ] `ADMIN_SECRET` лежит в менеджере паролей и никогда не пересылался в переписке
      — админка открыта из интернета, и это единственный фактор входа. Закрыть
      обратно: вернуть `respond @admin 404` в `Caddyfile`, см. комментарий в файле
- [ ] Все секреты в `.env` сгенерированы `openssl rand`, файл не в git
- [ ] PIN принимается только с устройства с device-cookie, `KIOSK_OPEN_ACCESS=false`
...
`Timeout during connect (likely firewall problem)` в логах означает, что пакеты
до сервера не доходят вовсе, и `ufw` тут ни при чём: у облачных провайдеров есть
свой фильтр перед ОС — в Oracle Cloud это security list у VCN, в Hetzner и AWS
firewall/security group. Открывать порты надо и там. Отличить это от блокировки
на самом сервере можно по счётчикам пакетов: обнули их, постучись снаружи и
посмотри, дошло ли хоть что-то до DNAT-правила Docker.

```bash
sudo iptables -t nat -Z DOCKER && nc -z <ip> 443; sudo iptables -t nat -L DOCKER -n -v | grep 443
```

Ноль пакетов — дело во внешнем фильтре. Есть пакеты, но соединение не встаёт —
разбирайся на сервере.

Отдельная ловушка Oracle Cloud: в их образах Ubuntu уже лежат правила iptables
(`/etc/iptables/rules.v4`) с `REJECT` в конце `INPUT` и `FORWARD`. На порты,
опубликованные Docker, они не влияют — те приходят через DNAT в `FORWARD` до
`REJECT`, — но любой сервис, поднятый на хосте напрямую, окажется недоступен, и
`ufw allow` этого не изменит.

**Правка `Caddyfile` не применяется после `git pull`.** `caddy reload` отвечает
`config is unchanged`, хотя в файле на хосте всё поправлено. Файл смонтирован в
контейнер по отдельности, а `git pull` заменяет его новым inode — контейнер
продолжает видеть старый. Лечится `up -d --force-recreate caddy`, подробнее в
разделе 7. Проверить, что видит контейнер:
...
**Машина зависла в статусе «занята» после долгого простоя.** Так быть не должно:
